...
function buildSearchPrompt(question, sources, knowledgeSources) {
  return [
    {
      role: "system",
      content: "你是智能运维搜索助手。请只根据提供的知识源线索和问题回答，输出简洁答案、处理建议和引用来源。"
    },
    {
      role: "user",
...
export async function answerQuestion(payload = {}) {
  ...
  const knowledgeSources = store.knowledge.map((item) => ({
    id: item.id,
    name: item.name,
    chunks: item.chunks
  }));
  const modelConfig = store.modelConfigs.find((item) => item.id === payload.modelConfigId) || store.modelConfigs[0];
  const modelResponse = await callConfiguredModel(modelConfig, buildSearchPrompt(question, sources, knowledgeSources), {
  ...
  return {
    ...
    modelProvider: modelResponse.used ? "live" : "local",
    modelFallbackReason: modelResponse.used ? "" : modelResponse.reason,
    knowledgeSources
  };
}
