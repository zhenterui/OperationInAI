...
export function runBusinessFlow(input = {}) {
  ...
  const sources = flow.dataSourceIds
    .map((id) => store.dataSources.find((source) => source.id === id))
    .filter(Boolean);
  const rules = flow.ruleIds
  ...
  store.syncLogs.unshift({
    ...
    status: "success",
    fetchedRows: Math.max(1, sources.length * 4),
    cleanedRows: 1,
    failedRows: 0,
    durationMs: 1260,
    startedAt: flow.lastRunAt,
    message: `已编排 ${sources.length} 个数据源，执行 ${rules.length} 条规则，输出到 ${flow.businessName}。规则：${ruleText}`
  });
  return { flow, business, row, sources, rules };
}
